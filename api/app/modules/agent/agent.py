...
TOOLS = [
    ...
    {"name": "read_plan", "description": "Rasteriza plano PDF a 200 DPI con crops opcionales. IMPORTANTE: NO uses esta herramienta si el archivo adjunto es una imagen simple (JPEG, PNG, WebP) de una o pocas piezas manuscritas — en ese caso analizá la imagen directamente con tu visión nativa para extraer medidas y perforaciones. Usá read_plan solo para PDFs complejos de múltiples páginas o cuando necesites hacer crop de una zona específica.", "input_schema": {"type": "object", "properties": {"filename": {"type": "string"}, "crop_instructions": {"type": "array", "items": {"type": "object", "properties": {"label": {"type": "string"}, "x1": {"type": "integer"}, "y1": {"type": "integer"}, "x2": {"type": "integer"}, "y2": {"type": "integer"}}}}}, "required": ["filename"]}},
